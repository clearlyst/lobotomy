...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    window->DrawList->AddRectFilled(bb.Min, bb.Max, ImColor(11, 11, 11, 255), 3.f);

    window->DrawList->AddRectFilled(bb.Min, bb.Max, ImColor(g_Options.fuckingaccent[0], g_Options.fuckingaccent[1], g_Options.fuckingaccent[2], it_anim->second.active_rect_alpha/255.f), 3.f);

    window->DrawList->AddRect(bb.Min, bb.Max, ImColor(255, 255, 255, (15 + it_anim->second.active_outline_anim)), 3.f);

    PushStyleColor(ImGuiCol_Text, ImVec4(ImColor(255, 255, 255, 255)));
    RenderTextClipped(bb.Min + ImVec2(0, 1), bb.Max + ImVec2(0, 1), label, NULL, &label_size, ImVec2(0.5f, 0.5f));
    PopStyleColor();
...
bool ImGui::Checkbox(const char* label, bool* v, int minus)
{
    ...
    const ImRect total_bb(pos, pos + ImVec2(square_sz + 7 + label_size.x, (label_size.y > square_sz) ? label_size.y : square_sz));
    ...
    window->DrawList->AddRectFilled(total_bb.Min, total_bb.Min + ImVec2(square_sz, square_sz), *v ? ImColor(g_Options.fuckingaccent[0], g_Options.fuckingaccent[1], g_Options.fuckingaccent[2]) : ImGui::IsItemHovered() ? ImColor(200, 200, 200, 255) : ImColor(30, 30, 30, 255), 0.f);
    ...
    if (*v) {
        ...
        color = ImColor(200, 200, 200, 255);
    }
    else {
        color = ImColor(30,30,30,255);
    }

    PushStyleColor(ImGuiCol_Text, color);

    RenderText(ImVec2(total_bb.Max.x - label_size.x, (total_bb.Min.y + 1) - (label_size.y / 2)), label);
...
void ImGui::ProgressBar(float fraction, const ImVec2& size_arg, const char* overlay)
{
    ...
    ImVec2 pos = window->DC.CursorPos;
    ImRect bb(pos, pos + CalcItemSize(size_arg, CalcItemWidth(), 4));
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImRect bb(window->DC.CursorPos + ImVec2(7, 0), window->DC.CursorPos + ImVec2(w, label_size.y + 5 + style.FramePadding.y * 2.0f));
    ...
    bool popup_open = IsPopupOpen(popup_id);
    ...
    // Render shape
    const ImU32 frame_col = ImColor(30,30,30);
    ...
    {
        ImU32 bg_col = (popup_open || hovered) ? ImColor(30, 30, 30) : ImColor(30, 30, 30);
        ImU32 text_col = GetColorU32(ImGuiCol_Text);
        window->DrawList->AddRectFilled(ImVec2(value_x2, bb.Min.y), bb.Max, bg_col, style.FrameRounding);
        if (value_x2 + arrow_size - style.FramePadding.x <= bb.Max.x)
            RenderArrow(ImVec2(value_x2 + style.FramePadding.y, bb.Min.y + style.FramePadding.y),  popup_open ? ImGuiDir_Up : ImGuiDir_Down, 1.0f);
    }
    ...
    else
    {
        ...
        SetNextWindowSizeConstraints(ImVec2(w2, 0.0f), ImVec2(FLT_MAX, CalcMaxPopupHeightFromItemCount(popup_max_height_in_items)));
    }
...
{ // modified by qo0
    ...
    */
    ...
    RenderText(ImVec2(frame_bb.Max.x - CalcTextSize(value_buf).x - 1.0f, (total_bb.Min.y - 2.0f) + style.FramePadding.y), value_buf, value_buf_end);

    if (label_size.x > 0.0f)
        RenderText(ImVec2(frame_bb.Min.x, (total_bb.Min.y - 2.0f) + style.FramePadding.y), label);
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (ColorButton("##ColorButton", col_v4, flags, ImVec2(15, 14)))
        {
