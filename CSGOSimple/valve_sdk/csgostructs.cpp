...
{
	return GetCSWeaponData()->nWeaponType == WEAPONTYPE_GRENADE;
}

bool C_BaseCombatWeapon::IsGun()
{
	switch (GetCSWeaponData()->nWeaponType)
	{
	case WEAPONTYPE_C4:
		return false;
...
bool C_BaseCombatWeapon::IsKnife()
{
	...
	return GetCSWeaponData()->nWeaponType == WEAPONTYPE_KNIFE;
}
...
void C_BasePlayer::collect(PlayerBackup& backup) const {
	backup.m_fFlags = m_fFlags();
	backup.m_nTickBase = m_nTickBase();
	backup.m_vecVelocity = m_vecVelocity();
	backup.fl_stamina = fl_stamina();
	backup.m_flMaxspeed = m_flMaxspeed();
	backup.m_flSimulationTime = m_flSimulationTime();
	backup.m_vecOrigin = m_vecOrigin();
}

void C_BasePlayer::apply(const PlayerBackup& backup) {
	m_fFlags() = backup.m_fFlags;
	m_nTickBase() = backup.m_nTickBase;
	m_vecVelocity() = backup.m_vecVelocity;
	fl_stamina() = backup.fl_stamina;
	m_flMaxspeed() = backup.m_flMaxspeed;
	m_flSimulationTime() = backup.m_flSimulationTime;
	m_vecOrigin() = backup.m_vecOrigin;
}